"""
KALSHI WEATHER BETTING MODEL v9.3-CITY-UNCERTAINTY
===================================================
IMPROVED VERSION with SMART BET SELECTION + CITY-SPECIFIC UNCERTAINTY.

Key changes from v9.2:
1. CITY-SPECIFIC UNCERTAINTY:
   - Chicago/NYC: σ=3.5°F (volatile weather, higher uncertainty)
   - Miami/LA: σ=2.5°F (stable weather, lower uncertainty)
   - Austin: σ=2.8°F (moderate)
   Based on actual performance data showing overconfidence in volatile cities.

Previous v9 features kept:
...
# - Chicago: -$18.80 (50% WR) → volatile (lake effect), higher uncertainty
# - NYC: -$42.45 (40.7% WR) → volatile (coastal), higher uncertainty
CITY_UNCERTAINTY = {
    "chicago": 3.5,    # Lake effect + fronts → higher uncertainty
    "nyc": 3.5,        # Coastal + urban effects → higher uncertainty
    "miami": 2.5,      # Stable subtropical → lower uncertainty
    "austin": 2.8,     # Continental, moderate volatility
    "la": 2.5,         # Mediterranean climate → stable
}
...
def print_header():
    print("=" * 70)
    print("KALSHI WEATHER BETTING MODEL v9.3-CITY-UNCERTAINTY")
    print("(Smart Bet Selection + Kelly Criterion + NWS + City-Specific σ)")
    print("=" * 70)
...
def main():
    ...
    print(f"\n{'='*70}")
    print("ANALYSIS COMPLETE (v9.3-CITY-UNCERTAINTY)")
    print(f"{'='*70}")
