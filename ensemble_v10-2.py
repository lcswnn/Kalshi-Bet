"""
KALSHI BIN-BASED PREDICTION MODEL v1.3
======================================
UPDATED MODEL SOURCES FOR INDEPENDENCE

v1.3 Changes (from v1.2):

1. ✅ REPLACED GFS WITH ECMWF
   - GFS and Open-Meteo were redundant (Open-Meteo uses GFS)
   - ECMWF is the European model, often considered most accurate globally
   - Truly independent from US-based NWS/HRRR
   
2. ✅ REPLACED GENERIC OPEN-METEO WITH ICON
   - ICON is the German weather model (DWD)
   - Independent from both US models and ECMWF
   - Good performance for North America

3. ✅ NEW ENSEMBLE (4 independent sources):
   - NWS (40%) - Official US source, what Kalshi uses for settlement
   - HRRR (25%) - High-resolution US model, best for short-term
   - ECMWF (20%) - European model, best global accuracy
   - ICON (15%) - German model, independent verification

All previous v1.2 features retained:
...
# City confidence multipliers (from backtest)
...
def apply_calibration(raw_prob: float) -> float:
    """Apply calibration based on backtest findings."""
    if raw_prob < 0.20:
        return raw_prob
    elif raw_prob < 0.35:
        return min(raw_prob * 1.20, 0.45)
    elif raw_prob < 0.50:
        return min(raw_prob * 1.10, 0.55)
    elif raw_prob < 0.60:
        return raw_prob * 0.70
    elif raw_prob < 0.70:
        return raw_prob * 0.50
    else:
        return raw_prob * 0.40
...
def calculate_kelly_bet(edge: float, model_prob: float, market_prob: float, 
                        bankroll: float, remaining_city_budget: float) -> float:
    """Kelly with city exposure limit."""
    ...
    fractional_kelly = max(0, min(full_kelly * KELLY_FRACTION, MAX_BET_FRACTION))
    
    bet_size = bankroll * fractional_kelly
    bet_size = min(bet_size, remaining_city_budget)
    
    return round(bet_size, 2) if bet_size >= MIN_BET_SIZE else 0.0
...
def select_trades(city_key: str, distribution: BinDistribution, 
    ...
    """Select trades with entropy filtering and exposure limits."""
    ...
    for bin_info in distribution.bins:
        ...
        # === YES BET EVALUATION ===
        raw_edge = bin_info.edge_spread_adjusted
        
        if raw_edge > 0:
            adjustments = []
            adjusted_edge = raw_edge
            
            if bin_info.spread > 0.05:
                adjustments.append(f"spread_penalty: -{bin_info.spread * SPREAD_PENALTY_FACTOR:.1%}")
            ...
            if adjusted_edge >= MIN_EDGE_YES_BET:
                ...
                if bet_size > 0:
                    ...
                    trades.append(TradeRecommendation(
                        ...
                        raw_edge=bin_info.edge_raw, adjusted_edge=adjusted_edge, bet_size=bet_size,
        ...
        raw_no_edge = no_calibrated_prob - no_market_prob - (bin_info.spread * SPREAD_PENALTY_FACTOR)
...
def print_header(target_date):
    print("=" * 70)
    print("🎯 KALSHI BIN-BASED PREDICTION MODEL v1.3")
...
def print_recommendations(trades: List[TradeRecommendation], bankroll: float):
    print(f"\n{'='*70}")
    print("💰 BETTING RECOMMENDATIONS (v1.3)")
    print(f"{'='*70}")
...
def main():
    ...
    print(f"\n{'='*70}")
    print("✅ ANALYSIS COMPLETE (v1.3 - ECMWF/ICON Ensemble)")
    print(f"{'='*70}")
