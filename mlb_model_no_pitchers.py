"""
MLB Game Prediction Model v3 — No Pitcher Features (Avoiding Data Leakage)
============================================================
Changes from v2:
  - REMOVED all pitcher features (they were causing data leakage)
  - Pitcher Elo and pitcher season stats completely removed
  - Model now relies purely on team-level features

Architecture:
  - Elo power ratings (carry across seasons, MOV-adjusted)
  - Team rolling performance metrics
...
Usage:
    python mlb_model_no_pitchers.py
...
# ═══════════════════════════════════════════════════════════════════════════
# 2. PITCHER ELO REMOVED (was causing data leakage)
# ═══════════════════════════════════════════════════════════════════════════
# Decision pitchers are determined AFTER the game, making them leak the outcome.
# Proper implementation would require starting pitcher data unavailable in our dataset.
...
def compute_elo_features(raw_path="mlb_raw_games.csv"):
    """Compute team Elo from raw game data (NO pitcher Elo - avoiding leakage)."""
    ...
    print(f"  ✓ Team Elo for {len(elo_df)} games")
    return elo_df


# ═══════════════════════════════════════════════════════════════════════════
# 4. PITCHER SEASON STATS REMOVED (was causing data leakage)
# ═══════════════════════════════════════════════════════════════════════════
# Season-level pitcher stats contain future information when used naively.
# Proper implementation requires rolling pitcher stats calculated only through
# games before the prediction date.
...
def prepare_features(matchups_path="mlb_matchups_features.csv",
    ...
    """Load matchups, merge team Elo (NO pitcher features - avoiding data leakage)."""
...
def plot_results(all_results, all_preds):
    ...
    plt.tight_layout()
    plt.savefig("mlb_model_v3_results.png", dpi=150, bbox_inches="tight")
    print("\n✓ Saved mlb_model_v3_results.png")
...
if __name__ == "__main__":
    print("=" * 60)
    print("MLB Prediction Model v3 — No Pitcher Features")
    print("=" * 60)
    ...
    if preds:
        ...
        all_preds.to_csv("mlb_predictions_v3.csv", index=False)
        print("✓ Saved mlb_predictions_v3.csv")
